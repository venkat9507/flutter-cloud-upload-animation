...
class _Bubble  {
  _Bubble({this.color, this.direction, this.speed, this.size, this.initialPosition});
  final Color color;
...
class DataBackupCloudPage extends StatelessWidget {
  final Animation<double>  cloudOutAnimation;
   ...
   DataBackupCloudPage({this.progressAnimation,this.cloudOutAnimation});
...
class _CloudBubblePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(_Bubble _bubble in bubbles){
      final offset = Offset(size.width/2 + _bubble.direction * animation.value, size.height * 1.2* (1- animation.value) - _bubble.speed * animation.value + _bubble.initialPosition * (1- animation.value)
      );
